    ...
    >
      ...
      {show && (
        <div style={{
          position: 'absolute', bottom: '130%', left: '50%', transform: 'translateX(-50%)',
          background: '#1e293b', color: '#f1f5f9', padding: '8px 12px', borderRadius: 8,
          fontSize: 12, lineHeight: 1.5, whiteSpace: 'nowrap', maxWidth: 260,
          whiteSpace: 'normal', zIndex: 9999, boxShadow: '0 4px 12px rgba(0,0,0,0.3)',
          pointerEvents: 'none'
        }}>
          {text}
        </div>
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{
      ...
      letterSpacing: 0.5, ...styles[status]
    }}>
      {status}
    </span>
  );
...
function StatCard({ label, value, color, icon }) {
  ...
  return (
    <div style={{
      ...
      padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 4, minWidth: 130
    }}>
      <span style={{ fontSize: 22 }}>{icon}</span>
      <span style={{ fontSize: 28, fontWeight: 900, color: c.text, fontFamily: 'Archivo Black, sans-serif' }}>{value}</span>
      <span style={{ fontSize: 12, color: '#6b7280', fontWeight: 500 }}>{label}</span>
...
export default function ReplenishmentDashboard() {
  ...
  const [importResult, setImportResult] = useState(null);
  const [importedBy, setImportedBy] = useState('');
  const [editingLeadTime, setEditingLeadTime] = useState(null); // { id, value }
  const [savingLeadTime, setSavingLeadTime] = useState(false);
  const fileInputRef = useRef();
  ...
  useEffect(() => {
    fetch('/api/migrate-replenishment').catch(() => {});
  }, []);
  ...
  const handleFileSelect = async (e) => {
    ...
    if (!importedBy.trim()) {
      alert('Please enter your name in the "Imported by" field before uploading.');
      e.target.value = '';
      return;
    }
  ...
  // ── Export previous forecast ─────────────────────────────────────────────────
  const handleExportPrevious = async () => {
    try {
      const res = await fetch('/api/forecast/last');
      if (!res.ok) {
        const json = await res.json();
        alert(json.error || 'No previous forecast found');
        return;
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `forecast_backup_${new Date().toISOString().split('T')[0]}.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
  ...
  // ── Save lead time inline ───────────────────────────────────────────────────
  const saveLeadTime = async () => {
    if (!editingLeadTime) return;
    const val = parseInt(editingLeadTime.value);
    if (isNaN(val) || val < 1) {
      alert('Lead time must be a positive number');
      return;
    }
    setSavingLeadTime(true);
    try {
      ...
      alert('Error saving lead time: ' + err.message);
    } finally {
  ...
  const thStyle = {
    ...
    color: '#374151', background: '#f9fafb', borderBottom: '2px solid #e5e7eb',
    whiteSpace: 'nowrap', position: 'sticky', top: 0, zIndex: 10
  };
  const tdStyle = {
    padding: '10px 12px', fontSize: 12, color: '#374151', borderBottom: '1px solid #f3f4f6',
    whiteSpace: 'nowrap'
  };

  const rowBg = (p) => {
    if (p.safetyStatus === 'Critical') return '#fef2f2';
    if (p.safetyStatus === 'Attention') return '#fffbeb';
    return '#ffffff';
  };

  const rowBorderLeft = (p) => {
    if (p.projectedDaysOfStock < 45 && p.safetyStatus !== 'Critical')
      return '3px solid #f97316'; // Orange warning for projected risk
    if (p.safetyStatus === 'Critical') return '3px solid #dc2626';
    if (p.safetyStatus === 'Attention') return '3px solid #f59e0b';
    return '3px solid transparent';
  };
  ...
  if (error) return (
    <div style={{ padding: 32 }}>
      <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 12, padding: 24, color: '#dc2626' }}>
        ...
        <br /><br />
        <button onClick={fetchData} style={{ padding: '8px 16px', background: '#dc2626', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer' }}>
          Retry
        </button>
        <span style={{ marginLeft: 12, fontSize: 12, color: '#6b7280' }}>
          If this is your first time using this page, run the migration first:
          <a href="/api/migrate-replenishment" target="_blank" style={{ marginLeft: 6, color: '#2563eb' }}>
            /api/migrate-replenishment
          </a>
        </span>
      </div>
    </div>
    ...
    <div style={{ padding: '24px 32px', maxWidth: 1800, margin: '0 auto' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: 13 }}>
          ...
          {meta.calculatedAt && (
            <span style={{ marginLeft: 12, color: '#94a3b8' }}>
              Last updated: {new Date(meta.calculatedAt).toLocaleTimeString()}
            </span>
          )}
        </p>
      </div>
      ...
      <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, marginBottom: 24, display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 8, cursor: importing ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 600,
        ...
        {importResult && (
          <div style={{
            ...
            border: `1px solid ${importResult.type === 'success' ? '#86efac' : '#fca5a5'}`
          }}>
            {importResult.message}
          </div>
        )}
            ...
            <button
              ...
              style={{
                ...
                background: filterStatus === s
                  ? (s === 'Critical' ? '#dc2626' : s === 'Attention' ? '#d97706' : s === 'Safe' ? '#16a34a' : '#2563eb')
                  : 'white',
                color: filterStatus === s ? 'white' : '#374151'
              }}
            >
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 11, color: '#6b7280', flexWrap: 'wrap' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 10, height: 10, background: '#fef2f2', border: '1px solid #fca5a5', display: 'inline-block', borderRadius: 2 }}></span>
          Critical — stock &lt; 45 days
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 10, height: 10, background: '#fffbeb', border: '1px solid #fcd34d', display: 'inline-block', borderRadius: 2 }}></span>
          Attention — 45–90 days
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 10, height: 10, background: '#f0fdf4', border: '1px solid #86efac', display: 'inline-block', borderRadius: 2 }}></span>
          Safe — &gt; 90 days
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 10, height: 10, background: 'transparent', border: '3px solid #f97316', display: 'inline-block', borderRadius: 2 }}></span>
          Projected risk (even if currently Safe)
        </span>
        <span style={{ color: '#94a3b8' }}>
          ↑ Gap worse than -10d &nbsp;|&nbsp; ↓ Gap better than +10d
        </span>
      </div>
          ...
          <thead>
            <tr>
              ...
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Current physical stock in litres from the database">Real Stock (L)</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="AvgDailyDemand × LeadTime × 1.5 — Buffer stock to cover lead time with 50% safety margin">Safety Stock (L)</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Total litres sold (remove/Shopify transactions) in the last 30 days, divided by 30">Avg Daily (L/d)</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Forecast volume for 120 days imported from Salesforce Excel">Forecast 120d (L)</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Forecast 120 days ÷ 120 — daily rate from Salesforce forecast">Forecast Daily (L/d)</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="MAX(Avg Daily Demand, Forecast Daily) — uses the higher of the two demand signals as the planning rate">Projected Daily (L/d)</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Real Stock ÷ Projected Daily Demand — How many days until stock-out using the worst-case demand scenario">Projected Days</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Real Stock ÷ Avg Daily Demand — Days remaining based on actual historical consumption only">Days of Stock</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Projected Days − Days of Stock. Negative (↑ red) = forecast demand exceeds history, risk is worse than it looks. Positive (↓ green) = forecast is lower, safer than history suggests.">Gap</Tooltip>
              </th>
              <th style={thStyle}>
                <Tooltip text="Critical = stock < 45d | Attention = 45–90d | Safe = >90d (based on historical demand)">Safety Status</Tooltip>
              </th>
              <th style={{ ...thStyle, textAlign: 'right' }}>
                <Tooltip text="Supplier lead time in days. Click to edit inline.">Lead Time (d)</Tooltip>
              </th>
              <th style={thStyle}>Supplier</th>
            </tr>
          </thead>
              ...
              filtered.map(p => (
                ...
                >
                  ...
                  <td style={{ ...tdStyle, fontWeight: 600, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {p.name}
                    {p.noSalesData && (
                      <span title="No sales in last 30 days — using minimum demand of 0.1 L/d" style={{ marginLeft: 6, fontSize: 10, color: '#94a3b8', fontWeight: 400 }}>
                        (no sales)
                      </span>
                    )}
                  </td>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right', color: '#6b7280' }}>{fmt(p.safetyStockLevel, 1)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>{fmt(p.avgDailyDemand, 3)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', color: p.hasForecast ? '#2563eb' : '#d1d5db' }}>
                    {p.hasForecast ? fmt(p.forecast120Days, 1) : '—'}
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right', color: p.hasForecast ? '#2563eb' : '#d1d5db' }}>
                    {p.hasForecast ? fmt(p.forecastDaily, 3) : '—'}
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 600, color: '#1f2937' }}>{fmt(p.projectedDaily, 3)}</td>
                  <td style={{
                    ...tdStyle, textAlign: 'right', fontWeight: 700,
                    color: p.projectedDaysOfStock < 45 ? '#dc2626' : p.projectedDaysOfStock < 90 ? '#d97706' : '#16a34a'
                  }}>
                    {p.projectedDaysOfStock >= 9990 ? '∞' : fmt(p.projectedDaysOfStock, 0)}
                  </td>
                  <td style={{
                    ...tdStyle, textAlign: 'right', fontWeight: 600,
                    color: p.daysOfStockActual < 45 ? '#dc2626' : p.daysOfStockActual < 90 ? '#d97706' : '#16a34a'
                  }}>
                    {p.daysOfStockActual >= 9990 ? '∞' : fmt(p.daysOfStockActual, 0)}
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>
                  ...
                  <td style={tdStyle}>
                    <StatusBadge status={p.safetyStatus} />
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>
                    {editingLeadTime?.id === p.id ? (
                      <span style={{ display: 'flex', gap: 4, alignItems: 'center', justifyContent: 'flex-end' }}>
                        <input
                          ...
                          onChange={e => setEditingLeadTime({ ...editingLeadTime, value: e.target.value })}
                          onKeyDown={e => {
                            if (e.key === 'Enter') saveLeadTime();
                            if (e.key === 'Escape') setEditingLeadTime(null);
                          }}
                          autoFocus
                          style={{ width: 60, border: '1px solid #3b82f6', borderRadius: 6, padding: '2px 6px', fontSize: 12, textAlign: 'right' }}
                        />
                        <button
                          onClick={saveLeadTime}
                          disabled={savingLeadTime}
                          style={{ padding: '2px 8px', background: '#16a34a', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 11 }}
                        >✓</button>
                        <button
                          onClick={() => setEditingLeadTime(null)}
                          style={{ padding: '2px 8px', background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 11 }}
                        >✗</button>
                      </span>
                    ) : (
                      <span
                        onClick={() => setEditingLeadTime({ id: p.id, value: p.leadTime })}
                        title="Click to edit lead time"
                        style={{ cursor: 'pointer', padding: '2px 8px', borderRadius: 6, display: 'inline-block' }}
                        onMouseEnter={e => e.currentTarget.style.background = '#eff6ff'}
                        onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                      >
                        {p.leadTime}
                  ...
                  <td style={{ ...tdStyle, color: '#6b7280', maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {p.supplier || '—'}
                  </td>
                </tr>
              ))
      ...
      {/* ── Footer note ── */}
      <p style={{ marginTop: 16, fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>
        Demand based on last 30 days of transactions (remove + Shopify webhook). Min demand = 0.1 L/d when no sales recorded. Lead time default = 30 days (click to edit per product).
      </p>
    </div>
  );
}
